...
def BoltzmannRule(EnergiesState, stateThere, stateHere):
    if EnergiesState[stateThere] < EnergiesState[stateHere]:
        return(True)
    else:
        chance = np.random.rand()
        if convertEnthalpyToDistribution(EnergiesState[stateThere] - EnergiesState[stateHere], k, T) > chance:
            return(True)
        else:
            return(False)
...
def simulate_rule(stateEnergies,Nsteps):
        ...
        for i in np.arange(Nsteps-1):
            ...
            move=BoltzmannRule(stateEnergies, newState, currentState)
